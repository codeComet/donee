...
// Fetch unread notifications + last 5 read notifications for the current user (client-side)
export async function fetchNotifications() {
  const supabase = createClient()

  const [unreadRes, seenRes] = await Promise.all([
    supabase
      .from('notifications')
  ...
  return { unread: unreadRes.data ?? [], seen: seenRes.data ?? [] }
}
