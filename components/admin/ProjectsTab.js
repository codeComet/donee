...
function ProjectFormModal({ isOpen, onClose, initialData, users, onSubmit, loading }) {
  ...
  const inputClass = 'w-full text-sm border border-slate-200 dark:border-slate-600 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-300 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 placeholder:text-slate-400'
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div className="px-6 py-5 space-y-4">
            <p id="project-form-desc" className="sr-only">Project details form</p>
            <div>
    ...
    mutationFn: ({ id, form }) =>
      supabaseMutate(async (supabase) => {
        const { error } = await supabase
          .from('projects')
          .update({ name: form.name, description: form.description, color: form.color, pm_id: form.pm_id || null })
          .eq('id', id)
        if (error) throw error
      }),
    onSuccess: () => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
              <tr>
                <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Project</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">PM</th>
                <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Created</th>
                ...
                <tr key={project.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    {project.pm ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-600 dark:text-slate-300">{project.pm.full_name}</span>
                      </div>
                    ) : (
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      ...
                      <button
                        onClick={() =>
                          toggleArchive.mutate({ id: project.id, archive: !project.is_archived })
                        }
                        className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors"
